...
import { buildProductivitySummary } from "@/lib/data/productivity";
import {
  buildMonthRange,
  getRoleScope,
...
export async function GET(request: Request) {
  ...
  try {
    const summary = await buildProductivitySummary(supabase, {
    ...
    return NextResponse.json(
      { error: "Failed to fetch productivity summary", code: "INTERNAL_ERROR" },
      { status: 500 },
    );
  }
}
