...
export async function GET(request: Request) {
  ...
  const { data, error, count } = await listProductivityRecords(supabase, {
    from,
    to,
    department_id:
  ...
  if (error) {
    return NextResponse.json(
      { error: "Failed to fetch productivity records", code: "INTERNAL_ERROR" },
      { status: 500 },
    );
  }
...
export async function POST(request: Request) {
  ...
  const parsed = createProductivityRecordSchema.safeParse(body);
  if (!parsed.success) {
    return NextResponse.json(
      {
  ...
  const { data, error } = await createProductivityRecord(supabase, parsed.data, user.id);
  if (error) {
    if (error.code === "23505") {
      return NextResponse.json(
    ...
    return NextResponse.json(
      { error: "Failed to create productivity record", code: "INTERNAL_ERROR" },
      { status: 500 },
    );
  }
