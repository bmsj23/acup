...
import { buildIncidentSummary } from "@/lib/data/incidents";
import {
  buildMonthRange,
  getRoleScope,
...
export async function GET(request: Request) {
  ...
  try {
    const summary = await buildIncidentSummary(supabase, {
    ...
    return NextResponse.json(
      { error: "Failed to fetch incident summary", code: "INTERNAL_ERROR" },
      { status: 500 },
    );
  }
}
