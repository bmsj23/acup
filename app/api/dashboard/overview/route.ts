...
import { buildDashboardOverview } from "@/lib/data/dashboard";
import {
  buildMonthRange,
  getRoleScope,
...
export async function GET(request: Request) {
  ...
  try {
    const overview = await buildDashboardOverview(supabase, {
    ...
    return NextResponse.json(
      { error: "Failed to fetch dashboard overview", code: "INTERNAL_ERROR" },
      { status: 500 },
    );
  }
}
