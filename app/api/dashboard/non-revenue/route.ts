...
import { buildDashboardNonRevenueSummary } from "@/lib/data/dashboard";
import {
  buildMonthRange,
  getRoleScope,
...
export async function GET(request: Request) {
  ...
  const { data: departments, error: departmentsError } = await listScopedDepartments(
    supabase,
    {
      role,
      memberDepartmentIds,
  ...
  try {
    const summary = await buildDashboardNonRevenueSummary(supabase, {
    ...
    return NextResponse.json(
      { error: "Failed to fetch non-revenue dashboard", code: "INTERNAL_ERROR" },
      { status: 500 },
    );
  }
}
