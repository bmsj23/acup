...
import { buildTrainingSummary } from "@/lib/data/training";
import {
  buildMonthRange,
  getRoleScope,
...
export async function GET(request: Request) {
  ...
  try {
    const summary = await buildTrainingSummary(supabase, {
    ...
    return NextResponse.json(
      { error: "Failed to fetch training summary", code: "INTERNAL_ERROR" },
      { status: 500 },
    );
  }
}
